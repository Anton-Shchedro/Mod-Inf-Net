...
from Code.utils.dataloader_LungInf import get_cv_loader
...
def train(train_loader, model, optimizer, epoch, train_save):
    ...
    #if (epoch+1) % 10 == 0:
        #torch.save(model.state_dict(), save_path + 'Inf-Net-%d-%d.pth' % (epoch+1))
...
if __name__ == '__main__':
    ...
    for k in range(opt.k_size):
        ...
        test_loader, train_loader = get_cv_loader(image_root, gt_root, edge_root,opt.k_size,k,opt.testsize,
                              batchsize=opt.batchsize, trainsize=opt.trainsize, num_workers=opt.num_workers)
        total_step = len(train_loader)

        print("#"*20, "\n Start Fold ", k+1, "/",opt.k_size)
        ...
        # ---- test -----
        test_model = model
        #test_model.load_state_dict(torch.load(opt.pth_path, map_location={'cuda:1': 'cuda:0'}))
        test_model.cuda()
        test_model.eval()


        for i in range(test_loader.size):
            image, name, gt = test_loader.load_data()

            image = image.cuda()

            lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2, lateral_edge = test_model(image)
            ...
            if(np.size(gt)==np.size(res)):
                err = np.count_nonzero(gt != res)/np.size(gt)
                error.append(err)
                # print(err)
            else:
                print("gt and res size are different")

        fold_error.append(np.sum(error)/len(error))
        print("avr err of fold ",np.sum(error)/len(error))



    for count, err in enumerate(fold_error):
        print(count,'     ', err)

    # ---- save best model ----

    torch.save(models[np.argmin(fold_error)].state_dict(),'./snapshots/save_weights/Inf-Net-Final.pth')
    print(np.min(fold_error))
    print('[Saving Snapshot:]', 'Inf-Net-%d.pth')